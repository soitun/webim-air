...
k.stopPropagation();k.cancelBubble=true}F(k)};z(O(b.actions.firstChild),function(k,l){aa(l,"ui-state-hover")});z(["minimize","maximize","close","resize"],function(k,l){t(b[l],"click",function(m){this.disabled||a[l]();d(m)});t(b[l],"mousedown",d)});if(s.runtime){var f=a.window.nativeWindow,h=a.window.document;a.window.htmlLoader.filters=s.runtime.Array(new s.runtime.flash.filters.DropShadowFilter(6,75,0,0.4,12,12));t(h,"keydown",function(){});t(b.headerTitle,"dblclick",function(){a._os.mac?a.minimize():
a.maximize()});t(b.header,"mousedown",function(){if(a.isMaximized())return false;f.startMove()});t(b.resize,"mousedown",function(){f.startResize(air.NativeWindowResize.BOTTOM_RIGHT)});if(a.options.main){t(air.NativeApplication.nativeApplication,air.InvokeEvent.INVOKE,function(){!f.visible&&(f.visible=true)});h=new air.Loader;t(h.contentLoaderInfo,air.Event.COMPLETE,function(k){air.NativeApplication.nativeApplication.icon.bitmaps=new runtime.Array(k.target.content.bitmapData)});h.load(new air.URLRequest(a.options.iconUrl))}t(f,
air.NativeWindowBoundsEvent.RESIZE,function(k){a.trigger("resize",k)});t(f,air.NativeWindowBoundsEvent.MOVE,function(k){a.trigger("move",k)});t(f,air.Event.ACTIVATE,function(k){a.trigger("activate",k)});t(f,air.Event.DEACTIVATE,function(k){a.trigger("deactivate",k)});t(f,air.Event.CLOSE,function(k){a.trigger("close",k)})}},activate:function(){return this.window&&this.window.nativeWindow.activate()},isActive:function(){return this.window&&this.window.nativeWindow.active},isMaximized:function(){return this.window&&
this.window.nativeWindow.displayState==air.NativeWindowDisplayState.MAXIMIZED},isMinimized:function(){return this.window&&this.window.nativeWindow.displayState==air.NativeWindowDisplayState.MINIMIZED},_os:function(){var a=s.runtime&&air.Capabilities.os;return{mac:/Mac/i.test(a),win:/Windows/i.test(a),linux:/Linux/i.test(a)}}()});L("layout",function(a){a=a||{};var b=this.im;layoutUI=new w.layout(a.layout,a.layoutOptions);this.addApp("login",C({container:layoutUI.element},a.loginOptions));this.addApp("user",
C({container:layoutUI.window.$.subHeader},a.userOptions));b.bind("go",function(){layoutUI.showContent()});layoutUI.window.subHeader(layoutUI.tabs.element);return layoutUI});I("layout",{template:'<div id=":layout" class="webim-layout webim-flex webim-box">\t\t<div id=":widgets" class="webim-widgets webim-flex webim-box webim-hide"></div>\t\t<div id=":shortcut" class="webim-shortcut ui-widget-header webim-hide"></div>\t</div>'},{_init:function(){C(this,{window:new w.window(null,{main:true,name:"layout",
title:"webim",maximizable:true,icon:""}),widgets:{},chats:{},tabs:new w.tabs(null,{panel:this.$.widgets}),chatWindows:{}});H(this.tabs.element,"webim-hide");this.window.html(this.element)},_initEvents:function(){},render:function(){u.body.appendChild(this.window.element)},widget:function(a){return this.widgets[a]},addWidget:function(a,b){var d=b.container,f=C(b,{main:false,closeable:false,subHeader:a.header}),h=a.element;this.widgets[a.name]=a;a.widget_title=b.title;if(d=="window"){d=new w.window(null,
f);d.html(h);a.container=d}else if(d=="tab"){this.tabs.add(a.element,b.title,b.icon);this.tabs.select(b.title)}},showWidget:function(a){this.tabs.select(this.widgets[a].widget_title)},focusChat:function(a,b){b=U(a,b);var d=this.chatWindows[b],f=this.chats[b];d&&d.activate();f&&f.focus()},chat:function(a,b){return this.chats[U(a,b)]},updateChat:function(a,b){b=R(b);for(var d,f=b.length,h,k=0;k<f;k++){d=b[k];(h=this.chats[U(a,d.id)])&&h.update(d)}},updateAllChat:function(){z(this.chats,function(a,b){b.update()})},
addChat:function(a,b,d){var f=this,h=f.chats;b=U(a,b);if(!h[b]){a=f.chatWindows[b]=(new w.window(null,{main:false,name:"chat",title:"webim",maximizable:true,icon:""})).bind("close",function(){delete f.chatWindows[b];delete f.chats[b]});d.setWindow(a);h[b]=d}},removeChat:function(a,b){var d=this.chatWindows[U(a,b)];d&&d.close()},removeAllChat:function(){z(this.chatWindows,function(a,b){b.close()})},showContent:function(){J(this.$.widgets,"webim-hide");J(this.tabs.element,"webim-hide");J(this.$.shortcut,
"webim-hide")}});I("history",{user:{},info:{},template:'<div class="webim-history">                        <div id=":content" class="webim-history-content">                 </div></div>'},{_init:function(){y.call(this,"init",[null,this.ui()])},clear:function(){this.$.content.innerHTML="";this.trigger("clear")},add:function(a){a=R(a);var b=a.length,d=[];if(b){for(var f=0;f<b;f++)d.push(this._renderMsg(a[f]));this.$.content.innerHTML+=d.join("");this.trigger("update")}},_renderMsg:function(a){a=C({},
a);y.call(this,"render",[null,this.ui({msg:a})]);var b=a.from,d=a.to,f=a.timestamp,h=a.body,k=true,l=this._lastLogItem,m=[],p;p=a.nick;if(l&&l.to==d&&l.from==b&&f-l.timestamp<6E4)k=false;if(k){this._lastLogItem=a;a=new Q(f);m.push('<h4><span class="webim-gray">');m.push(a.getDay(true));m.push(" ");m.push(a.getTime());m.push("</span>");m.push(p);m.push('</h4><hr class="webim-line ui-state-default" />')}m.push("<p>");m.push(h);m.push("</p>");return m.join("")},_renderDateBreak:function(a){var b=this._lastLogItem,
d=new Date,f=new Date,h=[];d.setTime(a);b&&f.setTime(b.timestamp);if(!b||d.getDate()!=f.getDate()||d.getMonth()!=f.getMonth()){h.push("<h5>");h.push((new Q(a)).getDay(true));h.push("</h5>")}return h.join("")},ui:function(a){return C({element:this.element,$:this.$},a)},plugins:{}});var G=function(){function a(f,h){return'<a href="'+(h=="www."?"http://"+f:f)+'"'+d+">"+f+"</a>"}function b(f,h){d+=" "+f+'="'+h+'"'}var d;return function(f,h){d="";h&&X(h)&&z(h,b);return f.replace(/(https?:\/\/|www\.)([^\s<]+)/ig,
a)}}();w.history.defaults.parseMsg=true;y.add("history","parseMsg",{render:function(a,b){var d=b.msg.body;d=$(d);d=G(d,{target:"_blank"});b.msg.body=d}});w.history.defaults.emot=true;y.add("history","emot",{render:function(a,b){b.msg.body=w.emot.parse(b.msg.body)}});I("emot",{template:'<div class="webim-emot ui-widget-content"><%=emots%></div>'},{_init:function(){var a=this,b=a.element;z(b.firstChild.childNodes,function(d,f){t(f,"click",function(){J(b,"webim-emot-show");a.trigger("select",this.firstChild.getAttribute("rel"))})})},
template:function(){var a=this.emots=webim.ui.emot.emots,b=[];b.push('<ul class="ui-helper-clearfix">');z(a,function(d,f){var h=f.src,k=f.t?f.t:f.q[0];b.push('<li><img src="');b.push(h);b.push('" title="');b.push(k);b.push('" alt="');b.push(f.q[0]);b.push('" rel="');b.push(f.q[0]);b.push('" /></li>')});b.push("</ul>");return o(this.options.template,{emots:b.join("")})},toggle:function(){A(this.element,"webim-emot-show")}});C(w.emot,{emots:[{t:"smile",src:"smile.png",q:[":)"]},{t:"smile_big",src:"smile-big.png",
