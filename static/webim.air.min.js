...
s(e,air.NativeWindowBoundsEvent.RESIZE,function(f){a.d("resize",f)});s(e,air.NativeWindowBoundsEvent.MOVE,function(f){a.d("move",f)});s(e,air.Event.ACTIVATE,function(f){a.d("activate",f)});s(e,air.Event.DEACTIVATE,function(f){a.d("deactivate",f)});s(e,air.Event.CLOSE,function(f){a.d("close",f)});s(e,air.NativeWindowDisplayStateEvent.DISPLAY_STATE_CHANGE,function(f){a.d("displayStateChange",f)})}},content:function(a){a?this.$.content.appendChild(a):this.$.content.innerHTML=""},header:function(a){a?
this.$.subHeader.appendChild(a):this.$.subHeader.innerHTML=""},title:function(a){this.$.headerTitle.innerHTML=a;this.options.title=a;this.window&&(this.window.nativeWindow.title=a)},icon:function(){this.options.title=title},notifyUser:function(a){air.NativeWindow.supportsNotification&&this.window&&this.window.notifyUser(a)},show:function(){this.window.nativeWindow.restore()},hide:function(){this.window.nativeWindow.visible=false},_changeState:function(a){var b=this.element;a="webim-window-"+(a=="restore"?
"normal":a);b&&(b.className=b.className.replace(RegExp("(^|\\s+)("+"webim-window-normal webim-window-maximize webim-window-minimize".split(/\s+/).join("|")+")(?=(\\s+|$))","g")," ")+" "+a)},maximize:function(){if(this.isMaximized())this.restore();else{this.window&&this.window.nativeWindow&&this.window.nativeWindow.maximize();this._changeState("maximize")}},restore:function(){this.window&&this.window.nativeWindow&&this.window.nativeWindow.restore();this._changeState("restore")},minimize:function(){if(!this.isMinimized()){this.window&&
this.window.nativeWindow&&this.window.nativeWindow.minimize();this._changeState("minimize")}},close:function(){if(this.options.main)this.hide();else this.window&&this.window.nativeWindow&&this.window.nativeWindow.close()},activate:function(){var a=this.window&&this.window.nativeWindow;if(a){self.show();a.restore();a.activate();a.orderToFront();air.NativeApplication.nativeApplication.activate(a)}},isActive:function(){return this.window&&this.window.nativeWindow.active},isMaximized:function(){return this.window&&
this.window.nativeWindow.displayState==air.NativeWindowDisplayState.MAXIMIZED},isMinimized:function(){return this.window&&this.window.nativeWindow.displayState==air.NativeWindowDisplayState.MINIMIZED},_os:function(){var a=t.runtime&&air.Capabilities.os;return{mac:/Mac/i.test(a),win:/Windows/i.test(a),linux:/Linux/i.test(a)}}()});g("layout",function(a){a=a||{};var b=this.im;layoutUI=new z.layout(a.layout,a.layoutOptions);this.addApp("login",B({container:layoutUI.element},a.loginOptions));this.addApp("user",
B({container:layoutUI.window.$.subHeader},a.userOptions));b.a("online",function(){layoutUI.showContent()});layoutUI.window.header(layoutUI.tabs.element);return layoutUI});d("layout",{template:'<div id=":layout" class="webim-layout webim-flex webim-box">\t\t<div id=":widgets" class="webim-widgets webim-flex webim-box webim-hide"></div>\t\t<div id=":shortcut" class="webim-shortcut ui-widget-header webim-hide"></div>\t</div>'},{_init:function(a,b){var c=this;b=c.options;B(c,{window:new z.window(null,
{main:true,name:"layout",title:"webim",maximizable:true,icon:""}),widgets:{},chats:{},tabs:new z.tabs(null,{panel:c.$.widgets}),chatWindows:{}});P(c.tabs.element,"webim-hide");c.window.content(c.element);if(t.runtime){var e=false,f=air.NativeApplication,l=f.nativeApplication,m=l.icon,n,r=(new DOMParser).parseFromString(l.applicationDescriptor,"text/xml"),p=function(){c.window.show()};if(f.supportsSystemTrayIcon){e=true;n=r.getElementsByTagName("image16x16")[0].textContent;s(m,"click",p);m.tooltip=
c.window.options.title}else if(f.supportsDockIcon){e=true;n=r.getElementsByTagName("image128x128")[0].textContent;s(l,air.InvokeEvent.INVOKE,p);c._badge=t.runtime.de&&t.runtime.de.mattesgroeger.air.icon.AirIconBadge}if(e){e=new air.Loader;s(e.contentLoaderInfo,air.Event.COMPLETE,function(v){v=v.target.content.bitmapData;m.bitmaps=new runtime.Array(v);try{c._badge&&(c._badge.customIcon=new runtime.flash.display.Bitmap(v))}catch(da){}});e.load(new air.URLRequest("app:/"+n));if(!f.supportsDockIcon){f=
new air.NativeMenu;n=new air.NativeMenuItem(u("logout"));s(n,air.Event.SELECT,function(){l.exit()});f.addItem(n);m.menu=f}}}setInterval(function(){c.notifyUser(air.NotificationType.INFORMATIONAL)},5E3);c.setBadge("3")},_initEvents:function(){},render:function(){x.body.appendChild(this.window.element)},widget:function(a){return this.widgets[a]},addWidget:function(a,b){var c=b.container,e=B(b,{main:false,closeable:false,subHeader:a.header}),f=a.element;this.widgets[a.name]=a;a.widget_title=b.title;
if(c=="window"){c=new z.window(null,e);c.content(f);a.container=c}else if(c=="tab"){this.tabs.add(a.element,b.title,b.icon);this.tabs.select(b.title)}},showWidget:function(a){this.tabs.select(this.widgets[a].widget_title)},focusChat:function(a,b){b=h(a,b);var c=this.chatWindows[b],e=this.chats[b];c&&c.activate();e&&e.focus()},chat:function(a,b){return this.chats[h(a,b)]},updateChat:function(a,b){b=Q(b);for(var c,e=b.length,f,l=0;l<e;l++){c=b[l];(f=this.chats[h(a,c.id)])&&f.update(c)}},updateAllChat:function(){D(this.chats,
